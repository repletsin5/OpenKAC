...
NTSTATUS BBSafeInitString( OUT PUNICODE_STRING result, IN PUNICODE_STRING source )
{
    ...
    if(source->Buffer && result->Buffer && source->Length)
...
__forceinline ULONG GenPrologue32( IN PUCHAR pBuf )
{
    *pBuf = 0x55;
...
__forceinline ULONG GenEpilogue32( IN PUCHAR pBuf, IN INT retSize )
{
    *(PUSHORT)pBuf = 0xEC89;
    *(pBuf + 2) = 0x5D;
    *(pBuf + 3) = 0xC2;
...
__forceinline ULONG GenCall32( IN PUCHAR pBuf, IN PVOID pFn, IN INT argc, ... )
{
    va_list vl;
    va_start( vl, argc );
...
__forceinline ULONG GenCall32V( IN PUCHAR pBuf, IN PVOID pFn, IN INT argc, IN va_list vl )
{
    ULONG ofst = 0;
    ...
    for (INT i = argc - 1; i >= 0; i--)
    {
        ...
        *(PULONG)(pBuf + ofst + 1) = pArgBuf[i];        // Ignore
        ofst += 5;
    }
...
__forceinline ULONG GenSync32( IN PUCHAR pBuf, IN PNTSTATUS pStatus, IN PVOID pSetEvent, IN HANDLE hEvent )
{
    ULONG ofst = 0;
...
__forceinline ULONG GenPrologue64( IN PUCHAR pBuf )
{
...
__forceinline ULONG GenEpilogue64( IN PUCHAR pBuf, IN INT retSize )
{
...
__forceinline ULONG GenCall64( IN PUCHAR pBuf, IN PVOID pFn, INT argc, ... )
{
    va_list vl;
    va_start( vl, argc );
...
__forceinline ULONG GenCall64V( IN PUCHAR pBuf, IN PVOID pFn, IN INT argc, IN va_list vl )
{
...
__forceinline ULONG GenSync64( IN PUCHAR pBuf, IN PNTSTATUS pStatus, IN PVOID pSetEvent, IN HANDLE hEvent )
{
    ULONG ofst = 0;
