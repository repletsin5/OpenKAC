...
PVOID GetKernelBase(OUT PULONG pSize)
{
    ...
    pMods = (PRTL_PROCESS_MODULES)ExAllocatePool2(POOL_FLAG_NON_PAGED_EXECUTE, bytes, BB_POOL_TAG);
    if (!pMods)
    {
        DPRINT("BlackBone: %s: ExAllocatePool2 failed \n", __FUNCTION__);
        return NULL;

    }
    //RtlZeroMemory( pMods, bytes ); //now 0 by default
