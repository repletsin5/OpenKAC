...
PVOID BBGetUserModule( IN PEPROCESS pProcess, IN PUNICODE_STRING ModuleName, IN BOOLEAN isWow64 )
{
    ...
    __try
    {
        ...
        if (isWow64)
        {
            DPRINT("BlackBone: %s: No Wow64 not supported. Aborting\n", __FUNCTION__);
            return NULL;
    ...
    )
{
    ...
    PKAPC pInjectApc = ExAllocatePool2( POOL_FLAG_NON_PAGED_EXECUTE, sizeof( KAPC ), BB_POOL_TAG );
...
NTSTATUS BBMapWorker( IN PVOID pArg )
{
    ...
    if (NT_SUCCESS( status ))
        //IDEA Create func that allocates mem and also check if ret is null and then fail after.
    ...
    if(fileData)
        ...
        DPRINT("BlackBone: %s: Failed to allocate memory \n", __FUNCTION__);
        return STATUS_NO_MEMORY; //should only return NULL with no memory 
