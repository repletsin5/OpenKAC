...
PVOID KpdbGetPDBStreamDirectory(PVOID base) {
	...
	PVOID stream_dir = ExAllocatePool2(POOL_FLAG_PAGED, block_count * block_size, 'kPDB');
...
StreamData* KpdbGetPDBStreams(PVOID base, PDWORD streams_count) {
	...
	StreamData* streams = (StreamData*)ExAllocatePool2(POOL_FLAG_PAGED, stream_num * sizeof(StreamData), 'kPDB');
	*streams_count = 0;
	if (!streams) {
		return 0;
	}
	for (int i = 0; i < stream_num; ++i) {
		if (stream_num == *streams_count) //Fixeed maybe possible out of bounds
			break;
		...
		current_stream = (PCHAR)ExAllocatePool2(POOL_FLAG_PAGED, current_stream_block_count * block_size, 'kPDB');
